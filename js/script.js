        ...
        form.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                window.dataLayer.push({
                    ...
                    'search_params': {
                        'jiku': payload.jiku + '番',
                        'aite_count': selectedAite.length + '頭',
                        'aite_list': selectedAite.length > 0 ? selectedAite.sort((a,b)=>a-b).join('_') + '番' : 'none',
                        ...
                        'distance': distType ? distanceMap[distType] : 'all',
                        'num_runners': runnersType ? runnersMap[runnersType] : 'all',
                        'year': payload.year ? payload.year + '年' : 'all',
                        'month': payload.month ? payload.month + '月' : 'all'
                    }
                });
